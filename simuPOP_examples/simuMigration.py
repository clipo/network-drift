...
def simuMigration(subPopSize, numOfSubPops, m, generations, numLoci):
    ...
    if useRPy:
        plotter = VarPlotter('[0.5] + [subPop[i]["alleleFreq"][0][0] for i in range(%d)]' % numOfSubPops,
                             ylim=[0, 1], update=generations - 1,
                             legend=['Theoretical'] + ['Allele freq at subpop %d' % x for x in range(numOfSubPops)],
...
if __name__ == '__main__':
    ...
    args.add_argument("--subPopSize",
                      default=5000,
                      type=int,
                      help="SubPopulation Size")
    args.add_argument("--numOfSubPops",
                      default=20,
                      type=int,
    ...
    simuMigration(args.subPopSize, args.numOfSubPops, args.m, args.generations, args.numLoci)
