...
class NetworkModel(object):
    ...
    def _parse_network_model(self):
        ...
        if self.networkmodel == "smallworld":
            ...
            self.network = network
        elif ".shp" in self.networkmodel:
            network = nx.read_shp(self.networkmodel)
            k = self.connectedness
            ...
            for ((x, y), data) in network.nodes(data=True):
                ...
                number += 1
            self.sub_pops = len(list(node_locations))
            self.subpopulation_names=list(node_locations.keys())
            ...
            for num, xy in list(node_locations.items()):
                node_distances = {}
                # now iterate through to find the n closest nodes
                ...
                for e in list_of_edges_to_add:
                    ...
                    if new_network.has_edge(num, ne):
                        ...
                        new_network.add_edge(num, ne, weight=(dist/exp_nearest_neighbor_distance)*self.migration_fraction)
