...
#Richness
##Figure 8. The impact of trait network structure and migration probability on trait richness within a population.  In this set of simulations, we divided populations of 5000 into a series of 200 subpopulations and varied connectivity from k=5 to k=190 in steps of 5. For each value of k, we ran the simulation with migration probabilities that ranged from 0.0001 to 0.005. The resulting data are the mean values of richness calculated from 10 runs at the point of 2000 timesteps. The richness of traits is best maintained under conditions of relatively low migration probability and low levels of k. 
...
#Figure 11. The impact of network structure is determined by connectivity and migration probability on the number of traits found only in one subpopulation after 2000 timesteps.
sweep_kvalues_mig_rare_trait_output <- read_csv(here("data/sweep-kvalues-mig","sweep-kvalues-mig-rare-trait-output.csv"),
        col_types = cols(InnovationRate = col_factor(), 
        Migration = col_character(), NumSubPops = col_factor(), 
        k = col_factor()))
...
ggsave("figure-11.pdf", plot = last_plot(), device = "pdf", 
  scale = 1,  dpi = 300)
```
Figure 11.   The impact of network structure is determined by the number of subpopulations (k=2) and migration probability on the number of traits found only in one subpopulation after 2000 timesteps.

```{r figure12, echo=FALSE}
#Figure 12.   The impact of network structure is determined by the number of subpopulations (k=2) and migration probability on the number of traits found only in one subpopulation after 2000 timesteps.
...
#Figure 16.  Trait richness after 2000 timesteps in the set of subpopulations modeled on Rapa Nui as a function of network connectivity (k) and migration probability. 
...
#Figure 17.  Traits found only in one subpopulation as a consequence of network connectivity (k) and migration probability after 2000 timesteps 
